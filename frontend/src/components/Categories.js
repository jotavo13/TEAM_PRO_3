...
function Categories({categories, setCategories, userID, videos, setVideos}) {
	...
	const onSubmitHandler = async (event) => {
		...
		const responseData = await fetch(`http://localhost:4000/${id}/categories/${newCategoryState}`, postOption);
	...
	if(videos === ''){
		...
		videoTitles = videos.map((video, index) => {
			return (
				<option className={index} id={video.id}>{video.title}</option>
			)
		})
	}
			...
			return (
				<li className="nav-item" >
					<a href="#" className="nav-link active" aria-current="page">
						...
						{category.name}
						<button className={index} onClick={hideAddCategory} id={category.name}>+</button>
					</a>
